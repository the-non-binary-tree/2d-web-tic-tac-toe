...
function Game(boardWidth, boardHeight) {
    ...
    this.move = function(mover, x, y) {
        if (this.winner != Winner.unidentified) {
            return;
        }
        if (!this.board[x][y]) {
    ...
    this.checkWinner = function(mover, x, y) {
        ...
        for (var i = 0; i < directions.length; i++) {
            ...
            if (totalCount >= winningRequirement && this.winner == Winner.unidentified) {
                this.winner = mover;
            }
        }
    };
...
function getWinnerAnnouncement(game, Winner, document) {
    var announcement;
    var winner = game.getWinner();
    if (winner == Winner.cross) {
        announcement = 'Winner is CROSS';
    } else if (winner == Winner.naught) {
        announcement = 'Winner is NAUGHT';
    } else if (winner == Winner.tie) {
        announcement = "It's a TIE";
    }
    document.getElementById('winnerAnnouncement').innerHTML = announcement;
...
canvas.addEventListener(
    'click', function main(evt) {
        ...
        if (winner != Winner.unidentified) {
            ...
            //state of user: if disabled (StateMachine) 
            //can also use for newGameButton to take the game to the pregame state
            // canvas.removeEventListener('click', main);
            getWinnerAnnouncement(game, Winner, document);
        }
    }, false
);
